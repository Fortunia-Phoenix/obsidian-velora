...
- 行为变化：彩虹文件夹改为按顶级文件夹路径的首个拉丁字母稳定映射 7 种颜色，不再依赖当前排序；新增、移动或重排文件夹不会让其他文件夹换色，重命名并改变首字母时仍会换色，非拉丁字母或符号开头使用粉色回退。
- 验证状态：本地合同测试已覆盖移除 `nth-child` 顺序依赖和基于 `data-path` 的稳定配色；实际 Obsidian 文件树视觉 QA 待运行时确认。

- 仓库整理：内部功能合同与回归测试改为仅在本地维护，GitHub 仓库只保留发布文件、用户说明和公开变更记录。
- 修复：右侧浮动侧边栏不再继承仅适用于窗口顶栏的 `126px` 窗口控制区占位，并将侧栏标签收敛为 `28px` 图标命中区；默认 `300px` 宽度现在可同时显示当前六个侧栏标签、原生“加号”和标签列表按钮，常规顶栏安全距离保持不变。
- 验证状态：本地合同测试与实际 Obsidian 1.13.7 DOM 布局检查均已通过；用户已于 2026-09-21 完成最终视觉确认。
...
- 验证状态：本地合同测试覆盖右侧开关、位置变量、镜像锚点、收回方向、把手、液态玻璃和减少动态效果；实际 Obsidian 双侧交互与视觉 QA 待运行时验证。
...
- 验证状态：扩展本地 PowerShell 合同测试，覆盖主题清单无 BOM、必需字段和设置按钮悬停位置；实际 Obsidian 1.13.7 重载后的主题列表与视觉状态仍待桌面 QA。
...
- 验证状态：本地回归测试已锁定 Style Settings 数值、Nexus 优先的 H2 五分支回退、桌面侧栏和减少动态效果合同；尚待实际 Obsidian 桌面视觉 QA。
